...
import { Search, Plus, Pencil, Package, AlertTriangle, Trash2, ChevronLeft, ChevronRight, ChevronUp, ChevronDown, Upload, X, CheckCircle, FolderTree } from "lucide-react";
...
function precioVentaEsperadoPorMargenGlobal(costo: number, gananciaPct: number): number {
  return parseFloat((costo * (1 + gananciaPct / 100)).toFixed(2));
}

const PRECIO_IGUAL_EPS = 0.02;
...
function textoPorcentajeVenta(
  ...
  if (Math.abs(precioVenta - esperado) <= PRECIO_IGUAL_EPS) {
    const g = gananciaGlobal;
    const pctStr = Math.abs(g - Math.round(g)) < 1e-6 ? String(Math.round(g)) : g.toFixed(1).replace(/\.0$/, "");
    return {
      texto: `${pctStr} %`,
...
export default function InventarioPage() {
  ...
  /** Margen % global (configuración); `null` hasta cargar desde la API */
