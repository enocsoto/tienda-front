...
/** Nombre del producto en la línea de venta (API envía `product.nombre`). */
function itemNombreProducto(it: SaleItemDisplay): string {
...
function formatCantidadItem(c: number | undefined): string {
...
function countSaleItems(sale: Sale): number {
            ...
            <div className="flex items-start justify-between gap-3 px-5 py-4 border-b border-slate-100 bg-slate-50/80">
              <div className="min-w-0">
                ...
                <p className="text-sm text-slate-500 mt-0.5">
                  {new Date(detailSale.fecha).toLocaleString("es-CO", {
                    dateStyle: "medium",
                    timeStyle: "short",
                  })}
                </p>
            ...
            <div className="px-5 py-4 overflow-y-auto flex-1 space-y-4">
              <dl className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <dd className="mt-0.5">
                    <Badge variant={detailSale.tipo?.toUpperCase() === "CONTADO" ? "emerald" : "amber"}>
                      {detailSale.tipo?.toUpperCase() ?? detailSale.tipo}
                    </Badge>
                  </dd>
                </div>
                <div>
                  <dt className="text-xs font-medium text-slate-400 uppercase tracking-wide">Método de pago</dt>
                  <dd className="mt-0.5 text-slate-800 capitalize">{detailSale.metodo_pago?.toLowerCase() ?? "—"}</dd>
                </div>
                <div>
                  <dt className="text-xs font-medium text-slate-400 uppercase tracking-wide">Cliente</dt>
                  <dd className="mt-0.5 text-slate-800">{detailSale.cliente_nombre || "—"}</dd>
                </div>
                <div>
                  <dt className="text-xs font-medium text-slate-400 uppercase tracking-wide">Teléfono</dt>
                  <dd className="mt-0.5 text-slate-800">{detailSale.cliente_telefono || "—"}</dd>
                </div>
                {detailSale.origen && (
                  <div>
                    <dt className="text-xs font-medium text-slate-400 uppercase tracking-wide">Origen</dt>
                    <dd className="mt-0.5 text-slate-800">{detailSale.origen}</dd>
                  </div>
                )}
                {detailSale.pago_confirmado === false && (
                ...
                {detailSale.direccion && (
                  <div className="sm:col-span-2">
                    <dt className="text-xs font-medium text-slate-400 uppercase tracking-wide">Entrega</dt>
                    <dd className="mt-0.5 text-slate-700">{detailSale.direccion}</dd>
                  </div>
                )}
                {detailSale.fecha_hora_entrega && (
                  <div className="sm:col-span-2">
                    <dt className="text-xs font-medium text-slate-400 uppercase tracking-wide">Fecha / hora entrega</dt>
                    <dd className="mt-0.5 text-slate-700">
                      {new Date(detailSale.fecha_hora_entrega).toLocaleString("es-CO")}
                    </dd>
                  </div>
                )}
                {detailSale.notas?.trim() && (
                  <div className="sm:col-span-2">
                    <dt className="text-xs font-medium text-slate-400 uppercase tracking-wide">Notas</dt>
                    <dd className="mt-0.5 text-slate-700 whitespace-pre-wrap">{detailSale.notas}</dd>
                  </div>
                )}
              </dl>
              ...
              <div className="flex justify-between items-center pt-2 border-t border-slate-100">
                ...
                <span className="text-lg font-bold text-sky-700">{formatCOP(Number(detailSale.total))}</span>
              </div>
            </div>
